...
public final class SystemServer {
    ...
    private static final TimingsTraceLog BOOT_TIMINGS_TRACE_LOG = new TimingsTraceLog(SYSTEM_SERVER_TIMING_TAG,
     ...
     * system-provided dialogs, such as the Power Off dialog, and other visual
     * content.
     */
    ...
    public SystemServer() {
        ...
        // Remember if it's runtime restart(when sys.boot_completed is already set) or
        // reboot
    ...
    private class AdbPortObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            int adbPort = LineageSettings.Secure.getInt(mContentResolver, LineageSettings.Secure.ADB_PORT, 0);
                ...
                if (packageFile.exists()) {
                    ...
                    if (filename != null && filename.startsWith("/data")) {
                        if (!new File(BLOCK_MAP_FILE).exists()) {
                            Slog.e(TAG,
                                    "Can't find block map file, uncrypt failed or " + "unexpected runtime restart?");
                            return;
                        }
                    }
                }
     ...
     * system off the ground. These services have complex mutual dependencies which
     * is why we initialize them all in one place here. Unless your service is also
     * entwined in these dependencies, it should be initialized in one of the other
     * functions.
     */
    private void startBootstrapServices() {
        ...
        // Manages A/B OTA dexopting. This is a bootstrap service as we need it to
        // rename
        ...
        mSensorServiceStart = SystemServerInitThreadPool.get().submit(() -> {
            TimingsTraceLog traceLog = new TimingsTraceLog(SYSTEM_SERVER_TIMING_ASYNC_TAG,
                    Trace.TRACE_TAG_SYSTEM_SERVER);
    ...
    private void startOtherServices() {
        ...
        String externalServer = context.getResources()
                .getString(org.lineageos.platform.internal.R.string.config_externalSystemServer);
        ...
        if (mFactoryTestMode != FactoryTest.FACTORY_TEST_LOW_LEVEL) {
            if (!disableStorage && !"0".equals(SystemProperties.get("system_init.startmountservice"))) {
                ...
                try {
                    /*
                     * NotificationManagerService is dependant on StorageManagerService, (for media
                     * / usb notifications) so we must start StorageManagerService first.
                     */
        ...
        if (mFactoryTestMode != FactoryTest.FACTORY_TEST_LOW_LEVEL) {
            ...
            if (!disableNetwork) {
                ...
                try {
                    networkPolicy = new NetworkPolicyManagerService(context, mActivityManagerService, networkStats,
                            networkManagement);
            ...
            notification = INotificationManager.Stub
            ...
            // timezone.RulesManagerService will prevent a device starting up if the chain
            // of trust
            // required for safe time zone updates might be broken. RuleManagerService
            // cannot do
            // this check when mOnlyCore == true, so we don't enable the service in this
            // case.
            // This service requires that JobSchedulerService is already started when it
            // starts.
            final boolean startRulesManagerService = !mOnlyCore
                    && context.getResources().getBoolean(R.bool.config_enableUpdateableTimeZoneRules);
        ...
        mContentResolver.registerContentObserver(LineageSettings.Secure.getUriFor(LineageSettings.Secure.ADB_PORT),
        ...
        mActivityManagerService.systemReady(() -> {
            ...
            try {
                ...
                final IIncidentManager incident = IIncidentManager.Stub
                        .asInterface(ServiceManager.checkService("incident"));
                if (incident != null)
