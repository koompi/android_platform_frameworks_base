...
final class DisplayDeviceInfo {
    /**
     * Flag: Indicates that this display device should be considered the default
     ...
     * physically rotates the display when content is presented in a different
     * orientation. The display manager will apply a coordinate transformation
     * assuming that the physical orientation of the display matches the logical
     * orientation of its content.
     * </p>
     * <p>
     * The flag should not be set when the display device is mounted in a fixed
     * orientation such as on a desk. The display manager will apply a coordinate
     * transformation such as a scale and translation to letterbox or pillarbox
     * format under the assumption that the physical orientation of the display is
     * invariant.
     * </p>
     */
    ...
    /**
     * Flag: Indicates that this display device has secure video output, such as
     * HDCP.
     */
    public static final int FLAG_SECURE = 1 << 2;

    /**
     * Flag: Indicates that this display device supports compositing from gralloc
     * protected buffers.
     */
     ...
     * and that no other application should be able to interact with it. Should
     * typically be used together with {@link #FLAG_OWN_CONTENT_ONLY}.
     */
    public static final int FLAG_PRIVATE = 1 << 4;

    /**
     * Flag: Indicates that the display device is not blanked automatically by the
     * power manager.
     */
    ...
    /**
     * Flag: Only show this display's own content; do not mirror the content of
     * another display.
     */
     ...
     * pionux
     *
     * Flag: Indicates that this display should be considered the default external
     * display.
     */
    ...
    /**
     * Touch attachment: Touch input is via an input device matching
     * {@link VirtualDisplay}'s uniqueId.
     * 
     * @hide
     */
    ...
    /**
     * Gets the name of the display device, which may be derived from EDID or other
     * sources. The name may be localized and displayed to the user.
     */
    public String name;
    ...
    /**
     * The width of the display in its natural orientation, in pixels. This value is
     * not affected by display rotation.
     */
    public int width;

    /**
     * The height of the display in its natural orientation, in pixels. This value
     * is not affected by display rotation.
     */
    public int height;
    ...
    /**
     * The nominal apparent density of the display in DPI used for layout
     * calculations. This density is sensitive to the viewing distance. A big TV and
     * a tablet may have the same apparent density even though the pixels on the TV
     * are much bigger than those on the tablet.
     */
    public int densityDpi;

    /**
     * The physical density of the display in DPI in the X direction. This density
     * should specify the physical size of each pixel.
     */
    public float xDpi;
    ...
    /**
     * This is a positive value indicating the phase offset of the VSYNC events
     * provided by Choreographer relative to the display refresh. For example, if
     * Choreographer reports that the refresh occurred at time N, it actually
     * occurred at (N - appVsyncOffsetNanos).
     */
     ...
     * submit the buffer before (N - bufferDeadlineNanos).
     */
    ...
    /**
     * The additional rotation to apply to all content presented on the display
     * device relative to its physical coordinate system. Default is
     * {@link Surface#ROTATION_0}.
     * <p>
     * This field can be used to compensate for the fact that the display has been
     * physically rotated relative to its natural orientation such as an HDMI
     * monitor that has been mounted sideways to appear to be portrait rather than
     * landscape.
     * </p>
     */
    ...
    /**
     * Computes the difference between display device infos. Assumes other is not
     * null.
     */
    public int diff(DisplayDeviceInfo other) {
        ...
        if (!Objects.equal(name, other.name) || !Objects.equal(uniqueId, other.uniqueId) || width != other.width
                || height != other.height || modeId != other.modeId || defaultModeId != other.defaultModeId
                || !Arrays.equals(supportedModes, other.supportedModes)
                || !Arrays.equals(supportedColorModes, other.supportedColorModes)
                || !Objects.equal(hdrCapabilities, other.hdrCapabilities) || densityDpi != other.densityDpi
                || xDpi != other.xDpi || yDpi != other.yDpi || appVsyncOffsetNanos != other.appVsyncOffsetNanos
                || presentationDeadlineNanos != other.presentationDeadlineNanos || flags != other.flags
                || touch != other.touch || rotation != other.rotation || type != other.type
                || !Objects.equal(address, other.address) || ownerUid != other.ownerUid
