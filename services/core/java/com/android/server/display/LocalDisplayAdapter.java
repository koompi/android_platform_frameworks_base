 ...
 * <p>
 * Display adapters are guarded by the {@link DisplayManagerService.SyncRoot}
 * lock.
 * </p>
 */
final class LocalDisplayAdapter extends DisplayAdapter {
    ...
    private static final int[] BUILT_IN_DISPLAY_IDS_TO_SCAN = new int[] { SurfaceControl.BUILT_IN_DISPLAY_ID_MAIN,
    ...
    public LocalDisplayAdapter(DisplayManagerService.SyncRoot syncRoot, Context context, Handler handler,
            Listener listener) {
    ...
    private void tryConnectDisplayLocked(int builtInDisplayId) {
        ...
        if (displayToken != null) {
            ...
            if (activeColorMode < 0) {
                // We failed to get the active color mode. We don't bail out here since on the
                // next
                // configuration pass we'll go ahead and set it to whatever it was set to last
                // (or
            ...
            if (device == null) {
                // Display was added.
                device = new LocalDisplayDevice(displayToken, builtInDisplayId, configs, activeConfig, colorModes,
                        activeColorMode);
    ...
    private final class LocalDisplayDevice extends DisplayDevice {
        ...
        public LocalDisplayDevice(IBinder displayToken, int builtInDisplayId,
                SurfaceControl.PhysicalDisplayInfo[] physicalDisplayInfos, int activeDisplayInfo, int[] colorModes,
                int activeColorMode) {
        ...
        public boolean updatePhysicalDisplayInfoLocked(SurfaceControl.PhysicalDisplayInfo[] physicalDisplayInfos,
                int activeDisplayInfo, int[] colorModes, int activeColorMode) {
            ...
            for (int i = 0; i < physicalDisplayInfos.length; i++) {
                ...
                // configuration options are exposed via Display.Mode, it's possible that we
                // have
                ...
                // supported modes then check to see if we have one in the prior set of
                // supported
                // modes to reuse.
            ...
            if (findDisplayInfoIndexLocked(mDefaultModeId) < 0) {
                if (mDefaultModeId != 0) {
                    Slog.w(TAG,
                            "Default display mode no longer available, using currently" + " active mode as default.");
                }
        ...
        private boolean updateColorModesLocked(int[] colorModes, int activeColorMode) {
            ...
            boolean colorModesChanged = pendingColorModes.size() != mSupportedColorModes.size() || colorModesAdded;
            ...
            if (!mSupportedColorModes.contains(mActiveColorMode)) {
                if (mActiveColorMode != 0) {
                    Slog.w(TAG, "Active color mode no longer available, reverting" + " to default mode.");
        ...
        public DisplayDeviceInfo getDisplayDeviceInfoLocked() {
            if (mInfo == null) {
                ...
                mInfo.supportedColorModes = new int[mSupportedColorModes.size()];
                ...
                if (mBuiltInDisplayId == SurfaceControl.BUILT_IN_DISPLAY_ID_MAIN) {
                    ...
                    if (res.getBoolean(com.android.internal.R.bool.config_mainBuiltInDisplayIsRound)
                            || (Build.IS_EMULATOR && SystemProperties.getBoolean(PROPERTY_EMULATOR_CIRCULAR, false))) {
                    ...
                    mInfo.name = getContext().getResources()
                    ...
                    if (!res.getBoolean(com.android.internal.R.bool.config_localDisplaysMirrorContent)) {
                     ...
                     * pionux
                     *
                     * Debug hook to toggle pionux changes.
                     */
                    if (SystemProperties.getBoolean("persist.pionux.desktop.hdmi", true)) {
            ...
            if (stateChanged || brightnessChanged) {
                ...
                return new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (Display.isSuspendedState(oldState) || oldState == Display.STATE_UNKNOWN) {
                            if (!Display.isSuspendedState(state)) {
                                setDisplayState(state);
                                currentState = state;
                            } else if (state == Display.STATE_DOZE_SUSPEND || oldState == Display.STATE_DOZE_SUSPEND) {
                        ...
                        if ((state == Display.STATE_VR || currentState == Display.STATE_VR) && currentState != state) {
                    ...
                    private void setVrMode(boolean isVrEnabled) {
                        if (DEBUG) {
                            Slog.d(TAG,
                                    "setVrMode(" + "id=" + displayId + ", state=" + Display.stateToString(state) + ")");
                        }
                    ...
                    private void setDisplayState(int state) {
                        if (DEBUG) {
                            Slog.d(TAG, "setDisplayState(" + "id=" + displayId + ", state="
                                    + Display.stateToString(state) + ")");
                        }

                        Trace.traceBegin(Trace.TRACE_TAG_POWER, "setDisplayState(" + "id=" + displayId + ", state="
                                + Display.stateToString(state) + ")");
                        try {
                    ...
                    private void setDisplayBrightness(int brightness) {
                        if (DEBUG) {
                            Slog.d(TAG,
                                    "setDisplayBrightness(" + "id=" + displayId + ", brightness=" + brightness + ")");
                        }
                        ...
                        try {
                            ...
                            Trace.traceCounter(Trace.TRACE_TAG_POWER, "ScreenBrightness", brightness);
                        } finally {
                            Trace.traceEnd(Trace.TRACE_TAG_POWER);
                        }
                    }
        ...
        public boolean requestModeInTransactionLocked(int modeId) {
            ...
            if (physIndex < 0) {
                Slog.w(TAG, "Requested mode ID " + modeId + " not available," + " trying with default mode ID");
                modeId = mDefaultModeId;
        ...
        public boolean requestColorModeInTransactionLocked(int colorMode) {
            ...
            if (!mSupportedColorModes.contains(colorMode)) {
                Slog.w(TAG, "Unable to find color mode " + colorMode + ", ignoring request.");
                return false;
            }
    ...
    private static final class DisplayModeRecord {
        ...
        /**
         * Returns whether the mode generated by the given PhysicalDisplayInfo matches
         * the mode contained by the record modulo mode ID.
         *
         * Note that this doesn't necessarily mean the the PhysicalDisplayInfos are
         * identical, just that they generate identical modes.
         */
        public boolean hasMatchingMode(SurfaceControl.PhysicalDisplayInfo info) {
            ...
            return mMode.getPhysicalWidth() == info.width && mMode.getPhysicalHeight() == info.height
