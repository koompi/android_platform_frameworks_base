 ...
 * applications when the state changes, and so on.
 * </p>
 * <p>
 * The display manager service relies on a collection of {@link DisplayAdapter}
 * components, for discovering and configuring physical display devices attached
 * to the system. There are separate display adapters for each manner that
 * devices are attached: one display adapter for built-in local displays, one
 * for simulated non-functional displays when the system is headless, one for
 * simulated overlay displays used for development, one for wifi displays, etc.
 * </p>
 * <p>
 * Display adapters are only weakly coupled to the display manager service.
 * Display adapters communicate changes in display device state to the display
 * manager service asynchronously via a {@link DisplayAdapter.Listener}
 * registered by the display manager service. This separation of concerns is
 * important for two main reasons. First, it neatly encapsulates the
 * responsibilities of these two classes: display adapters handle individual
 * display devices whereas the display manager service handles the global state.
 * Second, it eliminates the potential for deadlocks resulting from asynchronous
 * display device discovery.
 * </p>
 *
 * <h3>Synchronization</h3>
 * <p>
 * Because the display manager may be accessed by multiple threads, the
 * synchronization story gets a little complicated. In particular, the window
 * manager may call into the display manager while holding a surface transaction
 * with the expectation that it can apply changes immediately. Unfortunately,
 * that means we can't just do everything asynchronously (*grump*).
 * </p>
 * <p>
 * To make this work, all of the objects that belong to the display manager must
 * use the same lock. We call this lock the synchronization root and it has a
 * unique type {@link DisplayManagerService.SyncRoot}. Methods that require this
 * lock are named with the "Locked" suffix.
 * </p>
 * <p>
 * Where things get tricky is that the display manager is not allowed to make
 * any potentially reentrant calls, especially into the window manager. We
 * generally avoid this by making all potentially reentrant out-calls
 * asynchronous.
 * </p>
 */
public final class DisplayManagerService extends SystemService {
    ...
    // NOTE: This is synchronized on while holding WindowManagerService.mWindowMap
    // so never call
    // into WindowManagerService methods that require mWindowMap while holding this
    // unless you are
    ...
    // and only tell applications about the existence of the default logical
    // display.
    // The display manager can still mirror content to secondary displays but
    // applications
    ...
    private final CopyOnWriteArrayList<DisplayTransactionListener> mDisplayTransactionListeners = new CopyOnWriteArrayList<DisplayTransactionListener>();
    ...
    // For now, this only applies to the built-in display but we may split it up
    // eventually.
    private int mGlobalDisplayBrightness = PowerManager.BRIGHTNESS_DEFAULT;

    // Set to true when there are pending display changes that have yet to be
    // applied
    ...
    // The stable device screen height and width. These are not tied to a specific
    // display, even
    // the default display, because they need to be stable over the course of the
    // device's entire
    // life, even if the default display changes (e.g. a new monitor is plugged into
    // a PC-like
    // device).
    ...
    // Persistent data store for all internal settings maintained by the display
    // manager service.
    ...
    // Lists of UIDs that are present on the displays. Maps displayId -> array of
    // UIDs.
    ...
    DisplayManagerService(Context context, Injector injector) {
        ...
        mDefaultDisplayDefaultColorMode = mContext.getResources()
                .getInteger(com.android.internal.R.integer.config_defaultDisplayDefaultColorMode);
    ...
    public void setupSchedulerPolicies() {
        // android.display and android.anim is critical to user experience and we should
        // make sure
        // it is not in the default foregroup groups, add it to top-app to make sure it
        // uses all the
        ...
        Process.setThreadGroupAndCpuset(AnimationThread.get().getThreadId(), Process.THREAD_GROUP_TOP_APP);
    }

    @Override
    public void onStart() {
        // We need to pre-load the persistent data store so it's ready before the
        // default display
        // adapter is up so that we have it's configuration. We could load it lazily,
        // but since
        // we're going to have to read it in eventually we may as well do it here rather
        // than after
                ...
                while (mLogicalDisplays.get(Display.DEFAULT_DISPLAY) == null || mVirtualDisplayAdapter == null) {
                    ...
                    if (delay <= 0) {
                        throw new RuntimeException("Timeout waiting for default display "
                                + "to be initialized. DefaultDisplay=" + mLogicalDisplays.get(Display.DEFAULT_DISPLAY)
    ...
    private void loadStableDisplayValuesLocked() {
        ...
        if (size.x > 0 && size.y > 0) {
            // Just set these values directly so we don't write the display persistent data
            // again
            // unnecessarily
            mStableDisplaySize.set(size.x, size.y);
        } else {
            final Resources res = mContext.getResources();
            final int width = res.getInteger(com.android.internal.R.integer.config_stableDeviceDisplayWidth);
            final int height = res.getInteger(com.android.internal.R.integer.config_stableDeviceDisplayHeight);
            if (width > 0 && height > 0) {
        ...
        synchronized (mTempDisplayStateWorkQueue) {
            try {
                ...
                synchronized (mSyncRoot) {
                    ...
                    Trace.traceBegin(Trace.TRACE_TAG_POWER, "requestGlobalDisplayState(" + Display.stateToString(state)
    ...
    private void stopWifiDisplayScanLocked(CallbackRecord record) {
        if (record.mWifiDisplayScanRequested) {
            ...
            if (--mWifiDisplayScanRequestCount == 0) {
                ...
                Slog.wtf(TAG, "mWifiDisplayScanRequestCount became negative: " + mWifiDisplayScanRequestCount);
    ...
    private int createVirtualDisplayInternal(IVirtualDisplayCallback callback, IMediaProjection projection,
            int callingUid, String packageName, String name, int width, int height, int densityDpi, Surface surface,
        ...
        synchronized (mSyncRoot) {
            ...
            DisplayDevice device = mVirtualDisplayAdapter.createVirtualDisplayLocked(callback, projection, callingUid,
                    packageName, name, width, height, densityDpi, surface, flags, uniqueId);
            if (device == null) {
                return -1;
            }
            ...
            Slog.w(TAG,
                    "Rejecting request to create virtual display " + "because the logical display was not created.");
    ...
    private void registerDefaultDisplayAdapters() {
        ...
        synchronized (mSyncRoot) {
            ...
            // 2D UI. We register virtual display adapter along side the main display
            // adapter
    ...
    private void registerOverlayDisplayAdapterLocked() {
        registerDisplayAdapterLocked(
                new OverlayDisplayAdapter(mSyncRoot, mContext, mHandler, mDisplayAdapterListener, mUiHandler));
    }

    private void registerWifiDisplayAdapterLocked() {
        if (mContext.getResources().getBoolean(com.android.internal.R.bool.config_enableWifiDisplay)
                || SystemProperties.getInt(FORCE_WIFI_DISPLAY_ENABLE, -1) == 1) {
            mWifiDisplayAdapter = new WifiDisplayAdapter(mSyncRoot, mContext, mHandler, mDisplayAdapterListener,
    ...
    private void handleDisplayDeviceChanged(DisplayDevice device) {
        synchronized (mSyncRoot) {
            ...
            if (diff == DisplayDeviceInfo.DIFF_STATE) {
                Slog.i(TAG,
                        "Display device changed state: \"" + info.name + "\", " + Display.stateToString(info.state));
            } else if (diff != 0) {
    ...
    private void configureColorModeLocked(LogicalDisplay display, DisplayDevice device) {
        if (display.getPrimaryDisplayDeviceLocked() == device) {
            ...
            if (colorMode == Display.COLOR_MODE_INVALID) {
                if ((device.getDisplayDeviceInfoLocked().flags & DisplayDeviceInfo.FLAG_DEFAULT_DISPLAY) != 0) {
    ...
    // If we've never recorded stable device stats for this device before and they
    // aren't
    // explicitly configured, go ahead and record the stable device stats now based
    // on the status
    ...
    // Updates all existing logical displays given the current set of display
    // devices.
    ...
    private void setDisplayPropertiesInternal(int displayId, boolean hasContent, float requestedRefreshRate,
            int requestedModeId, boolean inTraversal) {
        synchronized (mSyncRoot) {
            LogicalDisplay display = mLogicalDisplays.get(displayId);
            if (display == null) {
                return;
            ...
            if (display.hasContentLocked() != hasContent) {
                if (DEBUG) {
                    Slog.d(TAG, "Display " + displayId + " hasContent flag changed: " + "hasContent=" + hasContent
            ...
            if (requestedModeId == 0 && requestedRefreshRate != 0) {
                // Scan supported modes returned by display.getInfo() to find a mode with the
                // same
    ...
    private void setDisplayAccessUIDsInternal(SparseArray<IntArray> newDisplayAccessUIDs) {
        synchronized (mSyncRoot) {
            ...
            for (int i = newDisplayAccessUIDs.size() - 1; i >= 0; i--) {
                mDisplayAccessUIDs.append(newDisplayAccessUIDs.keyAt(i), newDisplayAccessUIDs.valueAt(i));
            }
        }
    }

    // Checks if provided UID's content is present on the display and UID has access
    // to it.
    ...
    private void configureDisplayInTransactionLocked(DisplayDevice device) {
        ...
        display.configureDisplayInTransactionLocked(device, info.state == Display.STATE_OFF, layerStackOverride);
    ...
    private void dumpInternal(PrintWriter pw) {
        ...
        synchronized (mSyncRoot) {
            ...
            for (int i = 0; i < callbackCount; i++) {
                ...
                pw.println("  " + i + ": mPid=" + callback.mPid + ", mWifiDisplayScanRequested="
    ...
    /**
     * This is the object that everything in the display manager locks on. We make
     * it an inner class within the {@link DisplayManagerService} to so that it is
     * clear that the object belongs to the display manager service and that it is a
     * unique object with a special purpose.
     */
    ...
    static class Injector {
        VirtualDisplayAdapter getVirtualDisplayAdapter(SyncRoot syncRoot, Context context, Handler handler,
    ...
    final class BinderService extends IDisplayManager.Stub {
        ...
        public void requestColorMode(int displayId, int colorMode) {
            mContext.enforceCallingOrSelfPermission(Manifest.permission.CONFIGURE_DISPLAY_COLOR_MODE,
        ...
        @Override // Binder call
        public int createVirtualDisplay(IVirtualDisplayCallback callback, IMediaProjection projection,
                String packageName, String name, int width, int height, int densityDpi, Surface surface, int flags,
                String uniqueId) {
            final int callingUid = Binder.getCallingUid();
            ...
            final long token = Binder.clearCallingIdentity();
            try {
                return createVirtualDisplayInternal(callback, projection, callingUid, packageName, name, width, height,
        ...
        private boolean canProjectVideo(IMediaProjection projection) {
            ...
            if (mContext.checkCallingPermission(
                    android.Manifest.permission.CAPTURE_VIDEO_OUTPUT) == PackageManager.PERMISSION_GRANTED) {
                return true;
            }
        ...
        private boolean canProjectSecureVideo(IMediaProjection projection) {
            ...
            return mContext.checkCallingPermission(
                    android.Manifest.permission.CAPTURE_SECURE_VIDEO_OUTPUT) == PackageManager.PERMISSION_GRANTED;
        }
    }
    ...
    private final class LocalService extends DisplayManagerInternal {
        ...
        public void initPowerManagement(final DisplayPowerCallbacks callbacks, Handler handler,
            ...
            synchronized (mSyncRoot) {
                ...
                mDisplayPowerController = new DisplayPowerController(mContext, callbacks, handler, sensorManager,
                        blanker);
            }
        }
        ...
        @Override
        public void setDisplayProperties(int displayId, boolean hasContent, float requestedRefreshRate,
                int requestedMode, boolean inTraversal) {
            setDisplayPropertiesInternal(displayId, hasContent, requestedRefreshRate, requestedMode, inTraversal);
        }
