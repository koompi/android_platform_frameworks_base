 ...
 * interface to hardware, multiple perspectives can be managed (sometimes in
 * parallel) to better make use of context.
 *
 * Currently, PIONUX defines a mobile and desktop perspective that can be run in
 * parallel depending on the computing context of the user.
 *
 * Concurrency is handled by locking global state. All changes to state and
 * subsequent side effects (e.g. dispatching lifecycle events to listeners or
 * updating UI) are run under a single locked transaction.
 *
 * Note that the actual perspective lifecycle actions are carried out via JNI
 * calls to a native perspectived daemon.
 */
public class PerspectiveService extends IPerspectiveService.Stub {
    ...
    private void scheduleUpdateDesktopInteractiveState(boolean enable) {
        mHandler.sendEmptyMessage(enable ? MSG_ENABLE_DESKTOP_INTERACTIVE : MSG_DISABLE_DESKTOP_INTERACTIVE);
    }

    private void updateDesktopInteractiveStateLocked(boolean enable) {
        SystemProperties.set(PROPERTY_PIONUXOS_DESKTOP_INTERACTIVE, String.valueOf(enable));
        boolean updateResult = nativeEnableInput(mNativeClient, enable);
        if (!updateResult) {
            Log.w(TAG, "Update desktop interactive state failed");
        } else {
            if (mResolver != null) {
                Settings.Secure.putInt(mResolver, Settings.Secure.SHOW_IME_WITH_HARD_KEYBOARD, enable ? 1 : 0);
            } else {
    ...
    private void updateDesktopStateLocked(int state) {
        ...
        boolean isPublicPresentationConnected = mDisplayListener != null
                && mDisplayListener.isPublicPresentationConnected();
        boolean isDesktopRunning = mDesktopState == Perspective.STATE_RUNNING;
        boolean shouldEnableDesktopInteractiveState = isPublicPresentationConnected && isDesktopRunning;
    ...
    private void updateDesktopState() {
        synchronized (mLock) {
            ...
            updateDesktopStateLocked(isRunning ? Perspective.STATE_RUNNING : Perspective.STATE_STOPPED);
        }
    }
    ...
    private boolean isDesktopRunningInternal() {
        synchronized (mLock) {
            boolean isRunning = nativeIsRunning(mNativeClient);
            int state = isRunning ? Perspective.STATE_RUNNING : Perspective.STATE_STOPPED;
            if (mDesktopState != state) {
                /*
                 * Woops, there is a discrepancy: the desktop changed state under our feet for
                 * some reason. The good thing is that this should be a rare case.
                 *
                 * This could be fixed by polling nativeIsRunning but that seems wasteful so
                 * we'll just wait for a client to force us to check.
                 *
                 * Schedule an update of our state on the system thread that will sync up our
                 * state (provided there aren't any other pending lifecycle tasks scheduled
                 * before us that sync us up before it even runs).
                 */
                Log.w(TAG, "Yikes, desktop unexpectedly changed state to " + Perspective.stateToString(state)
    ...
    private void registerCallbackInternal(final IPerspectiveServiceCallback callback, final int pid) {
        synchronized (mLock) {
    ...
    private void updateDesktopNotificationLocked() {
        if (mDesktopState == Perspective.STATE_STOPPED) {
            ...
            final Intent intent = Intent.makeRestartActivityTask(new ComponentName("com.android.settings",
                    "com.android.settings.Settings$DesktopDashboardActivity"));
            final PendingIntent pendingIntent = PendingIntent.getActivityAsUser(mContext, 0, intent, 0, null,
                    UserHandle.CURRENT);

            final Notification notification = new Notification.Builder(mContext, SystemNotificationChannels.ALERTS)
                    .setContentTitle(mContext.getText(title)).setContentText(mContext.getText(msg))
                    .setTicker(mContext.getText(title)).setSmallIcon(icon).setContentIntent(pendingIntent)
                    .setOngoing(true).setColor(mContext.getColor(color)).setVisibility(Notification.VISIBILITY_PUBLIC)
                    .build();
    ...
    /**
     * Defer most work here for a few reasons: (1) Avoid blocking Binder clients for
     * expensive tasks, particularly the UI thread (2) Avoid permission issues
     * running stuff under Binder client context
     */
    ...
    private class MDisplayListener implements DisplayManager.DisplayListener {
        // track the public presentation display id to check if it has been removed
        // later
