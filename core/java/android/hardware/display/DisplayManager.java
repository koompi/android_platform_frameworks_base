...
public final class DisplayManager {
     ...
     * </p>
     * <p>
     * This broadcast is only sent to registered receivers and can only be sent by
     * the system.
     * </p>
     * 
     * @hide
     ...
     * <p>
     * This category can be used to identify secondary displays that are suitable
     * for use as presentation displays such as HDMI or Wireless displays.
     ...
     * </p>
     * <p>
     * Creating a public virtual display that isn't restricted to own-content only
     * implicitly creates an auto-mirroring display. See
     * {@link #VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR} for restrictions on who is allowed
     * to create an auto-mirroring display.
     * </p>
     *
     ...
     *
     * <p>
     * A private virtual display belongs to the application that created it. Only
     * the a owner of a private virtual display and the apps that are already on
     * that display are allowed to place windows upon it. The private virtual
     * display also does not participate in display mirroring: it will neither
     * receive mirrored content from another display nor allow its own content to be
     * mirrored elsewhere. More precisely, the only processes that are allowed to
     * enumerate or interact with the private display are those that have the same
     * UID as the application that originally created the private virtual display or
     * as the activities that are already on that display.
     * </p>
     *
     ...
     * display in the {@link #DISPLAY_CATEGORY_PRESENTATION presentation display
     * category}. Applications may automatically project their content to
     ...
     * the {@link Display#FLAG_SECURE} display flag. The caller promises to take
     ...
     * This permission is reserved for use by system components and is not available
    ...
    /**
     * Virtual display flag: Only show this display's own content; do not mirror the
     * content of another display.
     *
     * <p>
     ...
     * the default display if they have no windows of their own. When this flag is
     * specified, the virtual display will only ever show its own content and will
     * be blanked instead if it has no windows.
     * </p>
     *
     * <p>
     * This flag is mutually exclusive with
     * {@link #VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR}. If both flags are specified then
     * the own-content only behavior will be applied.
     * </p>
     *
     * <p>
     * This behavior of this flag is implied whenever neither
     * {@link #VIRTUAL_DISPLAY_FLAG_PUBLIC} nor
     * {@link #VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR} have been set. This flag is only
     * required to override the default behavior when creating a public display.
     * </p>
     *
     * @see #createVirtualDisplay
     */
    ...
    /**
     * Virtual display flag: Allows content to be mirrored on private displays when
     * no content is being shown.
     *
     * <p>
     * This flag is mutually exclusive with
     * {@link #VIRTUAL_DISPLAY_FLAG_OWN_CONTENT_ONLY}. If both flags are specified
     * then the own-content only behavior will be applied.
     * </p>
     *
     * <p>
     * The behavior of this flag is implied whenever
     * {@link #VIRTUAL_DISPLAY_FLAG_PUBLIC} is set and
     * {@link #VIRTUAL_DISPLAY_FLAG_OWN_CONTENT_ONLY} has not been set. This flag is
     * only required to override the default behavior when creating a private
     * display.
     * </p>
     *
     * <p>
     ...
     * {@link android.Manifest.permission#CAPTURE_SECURE_VIDEO_OUTPUT} permission.
     * These permissions are reserved for use by system components and are not
     * available to third-party applications.
     *
     * Alternatively, an appropriate {@link MediaProjection} may be used to create
     * an auto-mirroring virtual display.
     * </p>
     *
     * @see #createVirtualDisplay
     */
    ...
    /**
     * Virtual display flag: Allows content to be displayed on private virtual
     * displays when keyguard is shown but is insecure.
     *
     * <p>
     * This might be used in a case when the content of a virtual display is
     * captured and sent to an external hardware display that is not visible to the
     * system directly. This flag will allow the continued display of content while
     * other displays will be covered by a keyguard which doesn't require providing
     * credentials to unlock. This means that there is either no password or other
     * authentication method set, or the device is in a trusted state -
     * {@link android.service.trust.TrustAgentService} has available and active
     * trust agent.
     * </p>
     * <p>
     ...
     * {@link #VIRTUAL_DISPLAY_FLAG_PUBLIC}. If both flags are specified then this
     * flag's behavior will not be applied.
     * </p>
     *
     * @see #createVirtualDisplay
     ...
     * @hide
     */
    // TODO: Update name and documentation and un-hide the flag. Don't change the
    // value before that.
    public static final int VIRTUAL_DISPLAY_FLAG_CAN_SHOW_WITH_INSECURE_KEYGUARD = 1 << 5;

    /**
     * Virtual display flag: Specifies that the virtual display can be associated
     * with a touchpad device that matches its uniqueId.
     *
     * @see #createVirtualDisplay
     * @hide
     */
    ...
    /**
     * Virtual display flag: Indicates that the orientation of this display device
     * is coupled to the rotation of its associated logical display.
     *
     * @see #createVirtualDisplay
     * @hide
     */
    ...
    /**
     * Virtual display flag: Indicates that the contents will be destroyed once the
     * display is removed.
     *
     * Public virtual displays without this flag will move their content to main
     * display stack once they're removed. Private vistual displays will always
     * destroy their content on removal even without this flag.
     *
     * @see #createVirtualDisplay
     * @hide
     */
     ...
     * @return The display object, or null if there is no valid display with the
     *         given id.
     */
     ...
     * <p>
     * When there are multiple displays in a category the returned displays are
     * sorted of preference. For example, if the requested category is
     * {@link #DISPLAY_CATEGORY_PRESENTATION} and there are multiple presentation
     * displays then the displays are sorted so that the first display in the
     * returned array is the most preferred presentation display. The application
     * may simply use the first display or allow the user to choose.
     * </p>
     *
     * @param category The requested display category or null to return all
     *                 displays.
    ...
    private Display getOrCreateDisplayLocked(int displayId, boolean assumeValid) {
        ...
        if (display == null) {
            ...
            final Context context = mContext.getDisplay().getDisplayId() == displayId ? mContext
    ...
    /**
     * Registers an display listener to receive notifications about when displays
     * are added, removed or changed.
     *
     * @param listener The listener to register.
     * @param handler  The handler on which the listener should be invoked, or null
     *                 if the listener should be invoked on the calling thread's
     *                 looper.
     *
    ...
    /**
     * pionux: enable screen mirroring
     *
     * @hide
     */
    ...
    /**
     * pionux: disable screen mirroring
     *
     * @hide
     */
    ...
    /**
     * pionux: check if screen mirroring is enabled or not
     *
     * @hide
     */
     ...
     * {@link #ACTION_WIFI_DISPLAY_STATUS_CHANGED} broadcast.
     * <p>
     ...
     * </p>
     *
     * @param deviceAddress The MAC address of the device to which we should
     *                      connect.
     * @hide
     */
     ...
     * words, we must already have successfully connected to the display at least
     * once and then not forgotten it.
     * </p>
     * <p>
     * Requires {@link android.Manifest.permission#CONFIGURE_WIFI_DISPLAY}.
     * </p>
     ...
     * Creates a virtual display.
     * <p>
     * The content of a virtual display is rendered to a {@link Surface} provided by
     * the application.
     * </p>
     * <p>
     * The virtual display should be {@link VirtualDisplay#release released} when no
     * longer needed. Because a virtual display renders to a surface provided by the
     * application, it will be released automatically when the process terminates
     * and all remaining windows on it will be forcibly removed.
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * As of {@link android.os.Build.VERSION_CODES#KITKAT_WATCH}, the surface may be
     * attached or detached dynamically using {@link VirtualDisplay#setSurface}.
     ...
     * @param width      The width of the virtual display in pixels, must be greater
     *                   than 0.
     * @param height     The height of the virtual display in pixels, must be
     *                   greater than 0.
     * @param densityDpi The density of the virtual display in dpi, must be greater
     *                   than 0.
     ...
     *                   {@link #VIRTUAL_DISPLAY_FLAG_PUBLIC},
     *                   {@link #VIRTUAL_DISPLAY_FLAG_PRESENTATION},
     *                   {@link #VIRTUAL_DISPLAY_FLAG_SECURE},
     *                   {@link #VIRTUAL_DISPLAY_FLAG_OWN_CONTENT_ONLY}, or
     *                   {@link #VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR}.
     * @param callback   Callback to call when the state of the
     *                   {@link VirtualDisplay} changes
     * @param handler    The handler on which the listener should be invoked, or
     *                   null if the listener should be invoked on the calling
     *                   thread's looper.
     * @return The newly created virtual display, or null if the application could
     *         not create the virtual display.
     *
     * @throws SecurityException if the caller does not have permission to create a
     *                           virtual display with the specified flags.
     */
    public VirtualDisplay createVirtualDisplay(@NonNull String name, int width, int height, int densityDpi,
            @Nullable Surface surface, int flags, @Nullable VirtualDisplay.Callback callback,
            @Nullable Handler handler) {
        return createVirtualDisplay(null /* projection */, name, width, height, densityDpi, surface, flags, callback,
                handler, null /* uniqueId */);
    }

    /** @hide */
    public VirtualDisplay createVirtualDisplay(@Nullable MediaProjection projection, @NonNull String name, int width,
            int height, int densityDpi, @Nullable Surface surface, int flags,
     ...
     *
     * This should really only be used for things like server-side filtering of
     * available applications. Most applications don't need the level of stability
     * guaranteed by this and should instead query either the size of the display
     * they're currently running on or the size of the default display.
     * 
     * @hide
     */
    @SystemApi
    ...
    public interface DisplayListener {
         ...
         * {@link DisplayManager#getDisplay} to get more information about the display.
         *
