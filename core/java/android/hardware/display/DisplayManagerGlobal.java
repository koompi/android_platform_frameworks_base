 ...
 * application process. You're probably looking for {@link DisplayManager}.
 *
 * @hide
 */
public final class DisplayManagerGlobal {
    ...
    // orientation change before the display info cache has actually been
    // invalidated.
    ...
    private final ArrayList<DisplayListenerDelegate> mDisplayListeners = new ArrayList<DisplayListenerDelegate>();
     ...
     * @return Information about the specified display, or null if it does not
     *         exist. This object belongs to an internal cache and should be treated
    ...
    public VirtualDisplay createVirtualDisplay(Context context, MediaProjection projection, String name, int width,
            int height, int densityDpi, Surface surface, int flags, VirtualDisplay.Callback callback, Handler handler,
            String uniqueId) {
        ...
        int displayId;
        try {
            displayId = mDm.createVirtualDisplay(callbackWrapper, projectionToken, context.getPackageName(), name,
                    width, height, densityDpi, surface, flags, uniqueId);
        } catch (RemoteException ex) {
            throw ex.rethrowFromSystemServer();
        }
        if (displayId < 0) {
