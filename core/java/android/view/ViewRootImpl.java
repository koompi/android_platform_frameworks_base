...
public final class ViewRootImpl implements ViewParent, View.AttachInfo.Callbacks, ThreadedRenderer.DrawCallbacks {
    ...
    /**
     * Set to false if we do not want to use the multi threaded renderer. Note that
     * by disabling this, WindowCallbacks will not fire.
     */
    private static final boolean USE_MT_RENDERER = true;

    /**
     * Set this system property to true to force the view hierarchy to render at 60
     * Hz. This can be used to measure the potential framerate.
     */
    ...
    /**
     * Maximum time we allow the user to roll the trackball enough to generate a key
     * event, before resetting the counters.
     */
    ...
    /**
     * Callback used to notify corresponding activity about override configuration
     * change and make sure that all resources are set correctly before updating the
     ...
     * activity. In that case we receive a call back from {@link ActivityThread} and
     * this flag is used to preserve the initial value.
     *
    ...
    /**
     * Signals that compatibility booleans have been initialized according to target
     * SDK versions.
     */
    ...
    /**
     * This list must only be modified by the main thread, so a lock is only needed
     * when changing the list or when accessing the list from a non-main thread.
     */
    ...
    /**
     * Whether the window lost focus during a previous traversal and has not yet
     * gained it back. Used to determine whether a WINDOW_STATE_CHANGE accessibility
     * events should be sent during traversal.
     */
     ...
     * from internal callbacks such as onMeasure, onPreDraw, onDraw and deferring
     * doDie() until later.
     */
    ...
    /*
     * Input event queue. Pending input events are input events waiting to be
     * delivered to the input stages and handled by the application.
     */
    ...
    /**
     * Notifies us that our child has been rebuilt, following a window preservation
     * operation. In these cases we keep the same DecorView, but the activity
     * controlling it is a different instance, and we need to update our callbacks.
     *
     * @hide
     */
        ...
        synchronized (this) {
            if (mView == null) {
                ...
                if (DEBUG_KEEP_SCREEN_ON
                        && (mClientWindowLayoutFlags & WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON) != 0
                ...
                try {
                    ...
                    res = mWindowSession.addToDisplay(mWindow, mSeq, mWindowAttributes, getHostVisibility(),
                            mDisplay.getDisplayId(), mAttachInfo.mContentInsets, mAttachInfo.mStableInsets,
                ...
                if (res < WindowManagerGlobal.ADD_OKAY) {
                    ...
                    case WindowManagerGlobal.ADD_BAD_SUBWINDOW_TOKEN:
                        throw new WindowManager.BadTokenException("Unable to add window -- token " + attrs.token
                                + " is not valid; is your activity running?");
                    case WindowManagerGlobal.ADD_NOT_APP_TOKEN:
                        throw new WindowManager.BadTokenException(
                                "Unable to add window -- token " + attrs.token + " is not for an application");
                    ...
                    case WindowManagerGlobal.ADD_MULTIPLE_SINGLETON:
                        throw new WindowManager.BadTokenException("Unable to add window " + mWindow
                                + " -- another window of type " + mWindowAttributes.type + " already exists");
                    case WindowManagerGlobal.ADD_PERMISSION_DENIED:
                        throw new WindowManager.BadTokenException("Unable to add window " + mWindow
                                + " -- permission denied for window type " + mWindowAttributes.type);
                    case WindowManagerGlobal.ADD_INVALID_DISPLAY:
                        throw new WindowManager.InvalidDisplayException(
                                "Unable to add window " + mWindow + " -- the specified display can not be found");
                    case WindowManagerGlobal.ADD_INVALID_TYPE:
                        throw new WindowManager.InvalidDisplayException("Unable to add window " + mWindow
                                + " -- the specified window type " + mWindowAttributes.type + " is not valid");
                    }
     ...
     * @param waitForCompletion If true, this will not return until the functor has
     *                          invoked. If false, the functor may be invoked
     *                          asynchronously.
     */
    ...
    private void enableHardwareAcceleration(WindowManager.LayoutParams attrs) {
        ...
        if (hardwareAccelerated) {
            ...
            if (fakeHwAccelerated) {
                ...
                final boolean hasSurfaceInsets = insets.left != 0 || insets.right != 0 || insets.top != 0
                        || insets.bottom != 0;
                final boolean translucent = attrs.format != PixelFormat.OPAQUE || hasSurfaceInsets;
                final boolean wideGamut = mContext.getResources().getConfiguration().isScreenWideColorGamut()
    ...
    void setLayoutParams(WindowManager.LayoutParams attrs, boolean newView) {
        synchronized (this) {
            ...
            if ((mWindowAttributesChangesFlag & WindowManager.LayoutParams.LAYOUT_CHANGED) != 0) {
            ...
            if ((attrs.softInputMode
                    & WindowManager.LayoutParams.SOFT_INPUT_MASK_ADJUST) == WindowManager.LayoutParams.SOFT_INPUT_ADJUST_UNSPECIFIED) {
    ...
    public void onMovedToDisplay(int displayId, Configuration config) {
        ...
        mDisplay = ResourcesManager.getInstance().getAdjustedDisplay(displayId, mView.getResources());
        mAttachInfo.mDisplayState = mDisplay.getState();
    ...
    private void invalidateRectOnScreen(Rect dirty) {
        ...
        final boolean intersected = localDirty.intersect(0, 0, (int) (mWidth * appScale + 0.5f),
     ...
     * is allowed through to allow quick reversal of the Activity Transition.
     *
     ...
     * start()'ed. These transitionsa re added by LayoutTransition itself when it
     * sets up animations. The setup happens during the layout phase of traversal,
     * which we want to complete before any of the animations are started (because
     * those animations may side-effect properties that layout depends upon, like
     * the bounding rectangles of the affected views). So we add the transition to
     * the list and it is started just prior to starting the drawing phase of
     * traversal.
     *
    ...
    /**
     * Notifies the HardwareRenderer that a new frame will be coming soon. Currently
     * only {@link ThreadedRenderer} cares about this, and uses this knowledge to
     * adjust the scheduling of off-thread animations
     */
    ...
    private boolean measureHierarchy(final View host, final WindowManager.LayoutParams lp, final Resources res,
        ...
        if (DEBUG_ORIENTATION || DEBUG_LAYOUT)
            Log.v(mTag, "Measuring " + host + " in display " + desiredWindowWidth + "x" + desiredWindowHeight + "...");
        ...
        if (lp.width == ViewGroup.LayoutParams.WRAP_CONTENT) {
            ...
            if (DEBUG_DIALOG)
                Log.v(mTag,
                        "Window " + mView + ": baseSize=" + baseSize + ", desiredWindowWidth=" + desiredWindowWidth);
            if (baseSize != 0 && desiredWindowWidth > baseSize) {
                ...
                if (DEBUG_DIALOG)
                    Log.v(mTag,
                            "Window " + mView + ": measured (" + host.getMeasuredWidth() + ","
                                    + host.getMeasuredHeight() + ") from width spec: "
                                    + MeasureSpec.toString(childWidthMeasureSpec) + " and height spec: "
                ...
                if ((host.getMeasuredWidthAndState() & View.MEASURED_STATE_TOO_SMALL) == 0) {
                    ...
                    if (DEBUG_DIALOG)
                        Log.v(mTag, "Window " + mView + ": next baseSize=" + baseSize);
                    childWidthMeasureSpec = getRootMeasureSpec(baseSize, lp.width);
                    performMeasure(childWidthMeasureSpec, childHeightMeasureSpec);
                    if (DEBUG_DIALOG)
                        Log.v(mTag, "Window " + mView + ": measured (" + host.getMeasuredWidth() + ","
    ...
    /* package */ WindowInsets getWindowInsets(boolean forceConstruct) {
        if (mLastWindowInsets == null || forceConstruct) {
            ...
            // For dispatch we preserve old logic, but for direct requests from Views we
            // allow to
            // immediately use pending insets.
            if (!forceConstruct
                    && (!mPendingContentInsets.equals(contentInsets) || !mPendingStableInsets.equals(stableInsets))) {
                contentInsets = mPendingContentInsets;
                stableInsets = mPendingStableInsets;
            }
            Rect outsets = mAttachInfo.mOutsets;
            if (outsets.left > 0 || outsets.top > 0 || outsets.right > 0 || outsets.bottom > 0) {
                contentInsets = new Rect(contentInsets.left + outsets.left, contentInsets.top + outsets.top,
                        contentInsets.right + outsets.right, contentInsets.bottom + outsets.bottom);
            }
            mLastWindowInsets = new WindowInsets(contentInsets, null /* windowDecorInsets */, stableInsets,
                    mContext.getResources().getConfiguration().isScreenRound(), mAttachInfo.mAlwaysConsumeNavBar);
        }
    ...
    private static boolean shouldUseDisplaySize(final WindowManager.LayoutParams lp) {
        return lp.type == TYPE_STATUS_BAR_PANEL || lp.type == TYPE_INPUT_METHOD || lp.type == TYPE_VOLUME_OVERLAY;
    }
    ...
    private void performTraversals() {
        ...
        if (mApplyInsetsRequested) {
            ...
            if (mLayoutRequested) {
                ...
                windowSizeMayChange |= measureHierarchy(host, lp, mView.getContext().getResources(), desiredWindowWidth,
                        desiredWindowHeight);
            }
        }
        ...
        boolean windowShouldResize = layoutRequested && windowSizeMayChange
                && ((mWidth != host.getMeasuredWidth() || mHeight != host.getMeasuredHeight())
                        || (lp.width == ViewGroup.LayoutParams.WRAP_CONTENT && frame.width() < desiredWindowWidth
                                && frame.width() != mWidth)
                        || (lp.height == ViewGroup.LayoutParams.WRAP_CONTENT && frame.height() < desiredWindowHeight
        ...
        if (mFirst || windowShouldResize || insetsChanged || viewVisibilityChanged || params != null
            ...
            try {
                if (DEBUG_LAYOUT) {
                    Log.i(mTag, "host=w:" + host.getMeasuredWidth() + ", h:" + host.getMeasuredHeight() + ", params="
                            + params);
                }
                ...
                if (DEBUG_LAYOUT)
                    Log.v(mTag, "relayout: frame=" + frame.toShortString() + " overscan="
                            + mPendingOverscanInsets.toShortString() + " content="
                            + mPendingContentInsets.toShortString() + " visible="
                            + mPendingVisibleInsets.toShortString() + " visible=" + mPendingStableInsets.toShortString()
                            + " outsets=" + mPendingOutsets.toShortString() + " surface=" + mSurface);
                ...
                if (!mPendingMergedConfiguration.equals(mLastReportedMergedConfiguration)) {
                    if (DEBUG_CONFIGURATION)
                        Log.v(mTag, "Visible with new config: " + mPendingMergedConfiguration.getMergedConfiguration());
                ...
                if (contentInsetsChanged || mLastSystemUiVisibility != mAttachInfo.mSystemUiVisibility
                        || mApplyInsetsRequested || mLastOverscanRequested != mAttachInfo.mOverscanRequested
                        || outsetsChanged) {
                    mLastSystemUiVisibility = mAttachInfo.mSystemUiVisibility;
                    mLastOverscanRequested = mAttachInfo.mOverscanRequested;
                    ...
                    if (mSurface.isValid()) {
                        ...
                        if (mAttachInfo.mThreadedRenderer != null) {
                            try {
                                ...
                                if (hwInitialized
                                        && (host.mPrivateFlags & View.PFLAG_REQUEST_TRANSPARENT_REGIONS) == 0) {
                ...
                } else if ((surfaceGenerationId != mSurface.getGenerationId() || surfaceSizeChanged
                        || windowRelayoutWasForced) && mSurfaceHolder == null
                ...
                if (mDragResizing != dragResizing) {
                    if (dragResizing) {
                        ...
                        startDragResizing(mPendingBackDropFrame, mWinFrame.equals(mPendingBackDropFrame),
            ...
            if (threadedRenderer != null && threadedRenderer.isEnabled()) {
                if (hwInitialized || mWidth != threadedRenderer.getWidth() || mHeight != threadedRenderer.getHeight()
                        || mNeedsRendererSetup) {
            ...
            if (!mStopped || mReportNextDraw) {
                ...
                if (focusChangedDueToTouchMode || mWidth != host.getMeasuredWidth()
                    ...
                    if (DEBUG_LAYOUT)
                        Log.v(mTag,
                                "Ooops, something changed!  mWidth=" + mWidth + " measuredWidth="
                                        + host.getMeasuredWidth() + " mHeight=" + mHeight + " measuredHeight="
                                        + host.getMeasuredHeight() + " coveredInsetsChanged=" + contentInsetsChanged);
        ...
        if (didLayout) {
            ...
            if ((host.mPrivateFlags & View.PFLAG_REQUEST_TRANSPARENT_REGIONS) != 0) {
                ...
                mTransparentRegion.set(mTmpLocation[0], mTmpLocation[1], mTmpLocation[0] + host.mRight - host.mLeft,
        ...
        if (computesInternalInsets) {
            ...
            if (insetsPending || !mLastGivenInsets.equals(insets)) {
                ...
                try {
                    mWindowSession.setInsets(mWindow, insets.mTouchableInsets, contentInsets, visibleInsets,
                            touchableRegion);
                } catch (RemoteException e) {
                }
            }
        ...
        if (hasWindowFocus && !isInLocalFocusMode()) {
            ...
            if (imTarget != mLastWasImTarget) {
                ...
                if (imm != null && imTarget) {
                    ...
                    imm.onPostWindowFocus(mView, mView.findFocus(), mWindowAttributes.softInputMode,
     ...
     * view hierarchy is currently undergoing a layout pass.
     *
     ...
     * currently undergoing a layout pass. requestLayout() should not generally be
     * called during layout, unless the container hierarchy knows what it is doing
     * (i.e., it is fine as long as all children in that container hierarchy are
     * measured and laid out at the end of the layout pass for that container). If
     * requestLayout() is called anyway, we handle it correctly by registering all
     * requesters during a frame as it proceeds. At the end of the frame, we check
     * all of those views to see if any still have pending layout requests, which
     * indicates that they were not correctly handled by their container hierarchy.
     * If that is the case, we clear all such flags in the tree, to remove the buggy
     * flag state that leads to blank containers, and force a second
     * request/measure/layout pass in this frame. If more requestLayout() calls are
     * received during that second layout pass, we post those requests to the next
     * frame to avoid possible infinite loops.
     *
     * <p>
     * The return value from this method indicates whether the request should
     * proceed (if it is a request during the first layout pass) or should be
     * skipped and posted to the next frame (if it is a request during the second
     * layout pass).
     * </p>
     *
    ...
    private void performLayout(WindowManager.LayoutParams lp, int desiredWindowWidth, int desiredWindowHeight) {
        ...
        if (DEBUG_ORIENTATION || DEBUG_LAYOUT) {
            Log.v(mTag,
                    "Laying out " + host + " to (" + host.getMeasuredWidth() + ", " + host.getMeasuredHeight() + ")");
        }
        ...
        try {
            ...
            if (numViewsRequestingLayout > 0) {
                ...
                if (validLayoutRequesters != null) {
                    ...
                    measureHierarchy(host, lp, mView.getContext().getResources(), desiredWindowWidth,
                            desiredWindowHeight);
                    mInLayout = true;
                    host.layout(0, 0, host.getMeasuredWidth(), host.getMeasuredHeight());
     ...
     * requestLayout() during layout. It walks through the list of views that
     * requested layout to determine which ones still need it, based on visibility
     * in the hierarchy and whether they have already been handled (as is usually
     * the case with ListView children).
     *
     ...
     *                             second layout pass. If so, the FORCE_LAYOUT flag
     *                             was not set on requesters.
    ...
    private ArrayList<View> getValidLayoutRequesters(ArrayList<View> layoutRequesters, boolean secondLayoutRequests) {
        ...
        for (int i = 0; i < numViewsRequestingLayout; ++i) {
            ...
            if (view != null && view.mAttachInfo != null && view.mParent != null && (secondLayoutRequests
     ...
     *
     * @param rootDimension The layout params for one dimension (width or height) of
     *                      the window.
     *
    ...
    /**
     * A count of the number of calls to pendingDrawFinished we require to notify
     * the WM drawing is complete.
     */
    int mDrawsNeededToReport = 0;

    /**
     * Delay notifying WM of draw finished until a balanced call to
     * pendingDrawFinished.
     */
    void drawPending() {
    ...
    private void draw(boolean fullRedrawNeeded) {
        ...
        if (DEBUG_ORIENTATION || DEBUG_DRAW) {
            Log.v(mTag,
                    "Draw " + mView + "/" + mWindowAttributes.getTitle() + ": dirty={" + dirty.left + "," + dirty.top
                            + "," + dirty.right + "," + dirty.bottom + "} surface=" + surface + " surface.isValid()="
                            + surface.isValid() + ", appScale:" + appScale + ", width=" + mWidth + ", height="
                            + mHeight);
        }
        ...
        if (!dirty.isEmpty() || mIsAnimating || accessibilityFocusDirty) {
            if (mAttachInfo.mThreadedRenderer != null && mAttachInfo.mThreadedRenderer.isEnabled()) {
                ...
                if (mAttachInfo.mThreadedRenderer != null && !mAttachInfo.mThreadedRenderer.isEnabled()
                    ...
                    try {
                        mAttachInfo.mThreadedRenderer.initializeIfNeeded(mWidth, mHeight, mAttachInfo, mSurface,
                                surfaceInsets);
                    } catch (OutOfResourcesException e) {
                        handleOutOfResourcesException(e);
                        return;
     ...
     */
    private boolean drawSoftware(Surface surface, AttachInfo attachInfo, int xoff, int yoff, boolean scalingRequired,
            Rect dirty) {
        ...
        try {
            if (DEBUG_ORIENTATION || DEBUG_DRAW) {
                Log.v(mTag, "Surface " + surface + " drawing to bitmap w=" + canvas.getWidth() + ", h="
                        + canvas.getHeight());
            ...
            if (DEBUG_DRAW) {
                ...
                Log.i(mTag,
                        "Drawing: package:" + cxt.getPackageName() + ", metrics="
                                + cxt.getResources().getDisplayMetrics() + ", compatibilityInfo="
                                + cxt.getResources().getCompatibilityInfo());
            }
            try {
    ...
    /**
     * We want to draw a highlight around the current accessibility focused. Since
     * adding a style for all possible view is not a viable option we have this
     * specialized drawing method.
     *
     * Note: We are doing this here to be able to draw the highlight for virtual
     * views in addition to real ones.
     *
    ...
    private Drawable getAccessibilityFocusedDrawable() {
        ...
        if (mAttachInfo.mAccessibilityFocusDrawable == null) {
            ...
            final boolean resolved = mView.mContext.getTheme().resolveAttribute(R.attr.accessibilityFocusedDrawable,
                    value, true);
            if (resolved) {
     ...
     * draw, such that {@link WindowCallbacks#onPostDraw} gets called.
     */
        ...
        if (vi.left > ci.left || vi.top > ci.top || vi.right > ci.right || vi.bottom > ci.bottom) {
            ...
            if (focus == lastScrolledFocus && !mScrollMayChange && rectangle == null) {
                ...
                if (focus.getGlobalVisibleRect(mVisRect, null)) {
                    if (DEBUG_INPUT_RESIZE)
                        Log.v(mTag, "Root w=" + mView.getWidth() + " h=" + mView.getHeight() + " ci="
                                + ci.toShortString() + " vi=" + vi.toShortString());
                    if (rectangle == null) {
                        ...
                        if (DEBUG_INPUT_RESIZE)
                            Log.v(mTag, "Focus in window: focusRect=" + mTempRect.toShortString() + " visRect="
                                    + mVisRect.toShortString());
                    } else {
                        mTempRect.set(rectangle);
                        if (DEBUG_INPUT_RESIZE)
                            Log.v(mTag, "Request scroll to rect: " + mTempRect.toShortString() + " visRect="
                                    + mVisRect.toShortString());
                    }
    ...
    void setAccessibilityFocus(View view, AccessibilityNodeInfo node) {
        ...
        if ((mAccessibilityFocusedHost != null) && (mAccessibilityFocusedHost != view)) {
            ...
            mAccessibilityFocusedHost
                    .clearAccessibilityFocusNoCallbacks(AccessibilityNodeInfo.ACTION_ACCESSIBILITY_FOCUS);
        }
    ...
    private void performConfigurationChange(MergedConfiguration mergedConfiguration, boolean force, int newDisplayId) {
        ...
        if (DEBUG_CONFIGURATION)
            Log.v(mTag, "Applying new config to window " + mWindowAttributes.getTitle() + ", globalConfig: "
                    + globalConfig + ", overrideConfig: " + overrideConfig);
    ...
    final class ViewRootHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_RESIZED: {
                ...
                if (mWinFrame.equals(args.arg1) && mPendingOverscanInsets.equals(args.arg5)
                        && mPendingContentInsets.equals(args.arg2) && mPendingStableInsets.equals(args.arg6)
                        && mPendingVisibleInsets.equals(args.arg3) && mPendingOutsets.equals(args.arg7)
                        && mPendingBackDropFrame.equals(args.arg8) && args.arg4 == null && args.argi1 == 0
            ...
            case MSG_RESIZED_REPORT:
                if (mAdded) {
                    ...
                    final boolean framesChanged = !mWinFrame.equals(args.arg1)
                            || !mPendingOverscanInsets.equals(args.arg5) || !mPendingContentInsets.equals(args.arg2)
                            || !mPendingStableInsets.equals(args.arg6) || !mPendingVisibleInsets.equals(args.arg3)
    ...
    boolean ensureTouchMode(boolean inTouchMode) {
        if (DBG)
            Log.d("touchmode",
                    "ensureTouchMode(" + inTouchMode + "), current " + "touch mode is " + mAttachInfo.mInTouchMode);
    ...
    /**
     * Base class for implementing a stage in the chain of responsibility for
        ...
        protected boolean shouldDropInputEvent(QueuedInputEvent q) {
            ...
            if (mView == null || !mAdded) {
                ...
                return true;
                    ...
                    || mStopped || (mIsAmbientMode && !q.mEvent.isFromSource(InputDevice.SOURCE_CLASS_BUTTON))
     ...
     * <p>
     * In addition to what a normal input stage can do, an asynchronous input stage
     * may also defer an input event that has been delivered to it and finish or
     * forward it later.
     * </p>
     */
    abstract class AsyncInputStage extends InputStage {
         ...
         * 
         * @param next         The next stage to which events should be forwarded.
         * @param traceCounter The name of a counter to record the size of the queue of
         *                     pending events.
         */
         ...
         * asynchronously. The caller is responsible for calling {@link #forward} or
         * {@link #finish} later when it is done handling the event.
         */
    ...
    /**
     * Delivers pre-ime input events to a native activity. Does not support pointer
     * events.
     */
    ...
    /**
     * Delivers pre-ime input events to the view hierarchy. Does not support pointer
     * events.
     */
    ...
    final class EarlyPostImeInputStage extends InputStage {
        ...
        private int processPointerEvent(QueuedInputEvent q) {
            ...
            // Enter touch mode on down or scroll, if it is coming from a touch screen
            // device,
            // exit otherwise.
    ...
    final class ViewPostImeInputStage extends InputStage {
        ...
        @Override
        protected void onDeliverToNext(QueuedInputEvent q) {
            if (mUnbufferedInputDispatch && q.mEvent instanceof MotionEvent && ((MotionEvent) q.mEvent).isTouchEvent()
        ...
        private int processKeyEvent(QueuedInputEvent q) {
            ...
            if (event.getAction() == KeyEvent.ACTION_DOWN && !KeyEvent.metaStateHasNoModifiers(event.getMetaState())
                    && event.getRepeatCount() == 0 && !KeyEvent.isModifierKey(event.getKeyCode())
    ...
    final class SyntheticTrackballHandler {
        ...
        public void process(MotionEvent event) {
            ...
            case MotionEvent.ACTION_DOWN:
                mX.reset(2);
                mY.reset(2);
                enqueueInputEvent(new KeyEvent(curTime, curTime, KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_DPAD_CENTER, 0,
                        metaState, KeyCharacterMap.VIRTUAL_KEYBOARD, 0, KeyEvent.FLAG_FALLBACK,
                        InputDevice.SOURCE_KEYBOARD));
                break;
            case MotionEvent.ACTION_UP:
                mX.reset(2);
                mY.reset(2);
                enqueueInputEvent(new KeyEvent(curTime, curTime, KeyEvent.ACTION_UP, KeyEvent.KEYCODE_DPAD_CENTER, 0,
                        metaState, KeyCharacterMap.VIRTUAL_KEYBOARD, 0, KeyEvent.FLAG_FALLBACK,
                        InputDevice.SOURCE_KEYBOARD));
                break;
            }
            ...
            if (DEBUG_TRACKBALL)
                Log.v(mTag,
                        "TB X=" + mX.position + " step=" + mX.step + " dir=" + mX.dir + " acc=" + mX.acceleration
                                + " move=" + event.getX() + " / Y=" + mY.position + " step=" + mY.step + " dir="
                                + mY.dir + " acc=" + mY.acceleration + " move=" + event.getY());
            ...
            if (keycode != 0) {
                ...
                if (accelMovement > movement) {
                    ...
                    enqueueInputEvent(new KeyEvent(curTime, curTime, KeyEvent.ACTION_MULTIPLE, keycode, repeatCount,
                            metaState, KeyCharacterMap.VIRTUAL_KEYBOARD, 0, KeyEvent.FLAG_FALLBACK,
                            InputDevice.SOURCE_KEYBOARD));
                }
                while (movement > 0) {
                    ...
                    enqueueInputEvent(new KeyEvent(curTime, curTime, KeyEvent.ACTION_DOWN, keycode, 0, metaState,
                            KeyCharacterMap.VIRTUAL_KEYBOARD, 0, KeyEvent.FLAG_FALLBACK, InputDevice.SOURCE_KEYBOARD));
                    enqueueInputEvent(new KeyEvent(curTime, curTime, KeyEvent.ACTION_UP, keycode, 0, metaState,
                            KeyCharacterMap.VIRTUAL_KEYBOARD, 0, KeyEvent.FLAG_FALLBACK, InputDevice.SOURCE_KEYBOARD));
                }
                mLastTime = curTime;
            }
    ...
    /**
     * Maintains state information for a single trackball axis, generating discrete
     * (DPAD) movements based on raw trackball motion.
     */
    static final class TrackballAxis {
        ...
        /**
         * The maximum amount of time (in milliseconds) between events in order for us
         * to consider the user to be doing fast trackball movements, and thus apply an
         * acceleration.
         */
        static final long FAST_MOVE_TIME = 150;

        /**
         * Scaling factor to the time (in milliseconds) between events to how much to
         * multiple/divide the current acceleration. When movement is < FAST_MOVE_TIME
         * this multiplies the acceleration; when > FAST_MOVE_TIME it divides it.
         */
        ...
        /**
         * Add trackball movement into the state. If the direction of movement has been
         * reversed, the state is reset before adding the movement (so that you don't
         * have to compensate for any previously collected movement before see the
         * result of the movement in the new direction).
         *
         * @return Returns the absolute value of the amount of movement collected so
         *         far.
         */
            ...
            if (normTime > 0) {
                ...
                if (delta < normTime) {
                    ...
                    if (scale > 1)
                        acc *= scale;
                    if (DEBUG_TRACKBALL)
                        Log.v(TAG, axis + " accelerate: off=" + off + " normTime=" + normTime + " delta=" + delta
                                + " scale=" + scale + " acc=" + acc);
                    ...
                    if (scale > 1)
                        acc /= scale;
                    if (DEBUG_TRACKBALL)
                        Log.v(TAG, axis + " deccelerate: off=" + off + " normTime=" + normTime + " delta=" + delta
                                + " scale=" + scale + " acc=" + acc);
        ...
        /**
         * Generate the number of discrete movement events appropriate for the currently
         * collected trackball movement.
         *
         * @return Returns the number of discrete movements, either positive or
         *         negative, or 0 if there is not enough trackball movement yet for a
         *         discrete movement.
         */
        int generate() {
            int movement = 0;
    ...
    final class SyntheticJoystickHandler extends Handler {
        ...
        private void update(MotionEvent event, boolean synthesizeNewKeys) {
            ...
            if (xDirection != mLastXDirection) {
                if (mLastXKeyCode != 0) {
                    removeMessages(MSG_ENQUEUE_X_AXIS_KEY_REPEAT);
                    enqueueInputEvent(new KeyEvent(time, time, KeyEvent.ACTION_UP, mLastXKeyCode, 0, metaState,
                ...
                if (xDirection != 0 && synthesizeNewKeys) {
                    mLastXKeyCode = xDirection > 0 ? KeyEvent.KEYCODE_DPAD_RIGHT : KeyEvent.KEYCODE_DPAD_LEFT;
                    final KeyEvent e = new KeyEvent(time, time, KeyEvent.ACTION_DOWN, mLastXKeyCode, 0, metaState,
                            deviceId, 0, KeyEvent.FLAG_FALLBACK, source);
                    enqueueInputEvent(e);
            ...
            if (yDirection != mLastYDirection) {
                if (mLastYKeyCode != 0) {
                    removeMessages(MSG_ENQUEUE_Y_AXIS_KEY_REPEAT);
                    enqueueInputEvent(new KeyEvent(time, time, KeyEvent.ACTION_UP, mLastYKeyCode, 0, metaState,
                            deviceId, 0, KeyEvent.FLAG_FALLBACK, source));
                    mLastYKeyCode = 0;
                }
                ...
                if (yDirection != 0 && synthesizeNewKeys) {
                    mLastYKeyCode = yDirection > 0 ? KeyEvent.KEYCODE_DPAD_DOWN : KeyEvent.KEYCODE_DPAD_UP;
                    final KeyEvent e = new KeyEvent(time, time, KeyEvent.ACTION_DOWN, mLastYKeyCode, 0, metaState,
                            deviceId, 0, KeyEvent.FLAG_FALLBACK, source);
                    enqueueInputEvent(e);
    ...
    final class SyntheticTouchNavigationHandler extends Handler {
            ...
            if (mCurrentDeviceId != deviceId || mCurrentSource != source) {
                ...
                if (device != null) {
                    ...
                    if (xRange != null && yRange != null) {
                        ...
                        if (LOCAL_DEBUG) {
                            Log.d(LOCAL_TAG,
                                    "Configured device " + mCurrentDeviceId + " (" + Integer.toHexString(mCurrentSource)
        ...
        private void consumeAccumulatedMovement(long time, int metaState) {
            ...
            if (absX >= absY) {
                ...
                if (absY >= mConfigTickDistance) {
                    mAccumulatedY = consumeAccumulatedMovement(time, metaState, mAccumulatedY, KeyEvent.KEYCODE_DPAD_UP,
        ...
        private float consumeAccumulatedMovement(long time, int metaState, float accumulator, int negativeKeyCode,
                int positiveKeyCode) {
        ...
        private void sendKeyDownOrRepeat(long time, int keyCode, int metaState) {
            ...
            if (LOCAL_DEBUG) {
                Log.d(LOCAL_TAG, "Sending key down: keyCode=" + mPendingKeyCode + ", repeatCount="
                        + mPendingKeyRepeatCount + ", metaState=" + Integer.toHexString(mPendingKeyMetaState));
            }
            enqueueInputEvent(new KeyEvent(mPendingKeyDownTime, time, KeyEvent.ACTION_DOWN, mPendingKeyCode,
                    mPendingKeyRepeatCount, mPendingKeyMetaState, mCurrentDeviceId, KeyEvent.FLAG_FALLBACK,
                    mCurrentSource));
        }
        ...
        private void sendKeyUp(long time) {
            if (mPendingKeyCode != KeyEvent.KEYCODE_UNKNOWN) {
                if (LOCAL_DEBUG) {
                    Log.d(LOCAL_TAG, "Sending key up: keyCode=" + mPendingKeyCode + ", metaState="
                            + Integer.toHexString(mPendingKeyMetaState));
                }
                enqueueInputEvent(new KeyEvent(mPendingKeyDownTime, time, KeyEvent.ACTION_UP, mPendingKeyCode, 0,
                        mPendingKeyMetaState, mCurrentDeviceId, 0, KeyEvent.FLAG_FALLBACK, mCurrentSource));
        ...
        private boolean postFling(long time) {
            ...
            if (mFlingVelocity >= mConfigMinFlingVelocity) {
                ...
                if (LOCAL_DEBUG) {
                    Log.d(LOCAL_TAG, "Posted fling: velocity=" + mFlingVelocity + ", delay=" + delay + ", keyCode="
                            + mPendingKeyCode);
                }
                return true;
            }
    ...
    final class SyntheticKeyboardHandler {
        public void process(KeyEvent event) {
            ...
            if (fallbackAction != null) {
                ...
                KeyEvent fallbackEvent = KeyEvent.obtain(event.getDownTime(), event.getEventTime(), event.getAction(),
     ...
     * @return Whether this key event should be consumed (meaning the act of leaving
     *         touch mode alone is considered the event).
     */
    ...
    private int relayoutWindow(WindowManager.LayoutParams params, int viewVisibility, boolean insetsPending)
        ...
        if (params != null && mOrigWindowType != params.type) {
            ...
            if (mTargetSdkVersion < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                Slog.w(mTag,
                        "Window type can not be changed after " + "the window is added; ignoring change of " + mView);
                params.type = mOrigWindowType;
            }
        }
        int relayoutResult = mWindowSession.relayout(mWindow, mSeq, params,
                (int) (mView.getMeasuredWidth() * appScale + 0.5f), (int) (mView.getMeasuredHeight() * appScale + 0.5f),
                viewVisibility, insetsPending ? WindowManagerGlobal.RELAYOUT_INSETS_PENDING : 0, mWinFrame,
                mPendingOverscanInsets, mPendingContentInsets, mPendingVisibleInsets, mPendingStableInsets,
                mPendingOutsets, mPendingBackDropFrame, mPendingMergedConfiguration, mSurface);

        mPendingAlwaysConsumeNavBar = (relayoutResult & WindowManagerGlobal.RELAYOUT_RES_CONSUME_ALWAYS_NAV_BAR) != 0;
    ...
    public void playSoundEffect(int effectId) {
        ...
        try {
            ...
            case SoundEffectConstants.NAVIGATION_UP:
                ...
                return;
            default:
                throw new IllegalArgumentException("unknown effect id " + effectId + " not defined in "
                        + SoundEffectConstants.class.getCanonicalName());
            }
    ...
    public View keyboardNavigationClusterSearch(View currentCluster, @FocusDirection int direction) {
        checkThread();
        return FocusFinder.getInstance().findNextKeyboardNavigationCluster(mView, currentCluster, direction);
    }
    ...
    private void dispatchResized(Rect frame, Rect overscanInsets, Rect contentInsets, Rect visibleInsets,
            Rect stableInsets, Rect outsets, boolean reportDraw, MergedConfiguration mergedConfiguration,
            Rect backDropFrame, boolean forceLayout, boolean alwaysConsumeNavBar, int displayId) {
        if (DEBUG_LAYOUT)
            Log.v(mTag,
                    "Resizing " + this + ": frame=" + frame.toShortString() + " contentInsets="
                            + contentInsets.toShortString() + " visibleInsets=" + visibleInsets.toShortString()
                            + " reportDraw=" + reportDraw + " backDropFrame=" + backDropFrame);
        ...
        if (mDragResizing) {
            ...
            synchronized (mWindowCallbacks) {
                for (int i = mWindowCallbacks.size() - 1; i >= 0; i--) {
                    mWindowCallbacks.get(i).onWindowSizeIsChanging(backDropFrame, fullscreen, visibleInsets,
                            stableInsets);
                }
            }
        }
        ...
        args.arg4 = sameProcessCall && mergedConfiguration != null ? new MergedConfiguration(mergedConfiguration)
                : mergedConfiguration;
     ...
     * delivers one input event to the application at a time and waits for the
     * application to finish handling it before delivering the next one.
     *
    ...
    void unscheduleConsumeBatchedInput() {
        if (mConsumeBatchedInputScheduled) {
            ...
            mChoreographer.removeCallbacks(Choreographer.CALLBACK_INPUT, mConsumedBatchedInputRunnable, null);
        }
    }
            ...
            synchronized (this) {
                ...
                if (viewRectCount != 0) {
                    mTempViewRects = mViewRects.toArray(
                            mTempViewRects != null ? mTempViewRects : new AttachInfo.InvalidateInfo[viewRectCount]);
                    mViewRects.clear();
                }
            }
    ...
    public void updatePointerIcon(float x, float y) {
        ...
        final MotionEvent event = MotionEvent.obtain(0, now, MotionEvent.ACTION_HOVER_MOVE, x, y, 0);
        Message msg = mHandler.obtainMessage(what, event);
        mHandler.sendMessage(msg);
    }
    ...
    public void dispatchRequestKeyboardShortcuts(IResultReceiver receiver, int deviceId) {
        mHandler.obtainMessage(MSG_REQUEST_KEYBOARD_SHORTCUTS, deviceId, 0, receiver).sendToTarget();
    }
     ...
     * {@link AccessibilityEvent#TYPE_WINDOW_CONTENT_CHANGED} event. This event is
    ...
    /**
     * Updates the focused virtual view, when necessary, in response to a content
     * changed event.
     * <p>
     ...
     * SystemUI and window manager when waiting for the first frame to be drawn when
     * turning on the screen. DO NOT USE unless you fully understand this
     * interaction.
     * 
     * @hide
     */
    ...
    static class W extends IWindow.Stub {
        ...
        @Override
        public void resized(Rect frame, Rect overscanInsets, Rect contentInsets, Rect visibleInsets, Rect stableInsets,
                Rect outsets, boolean reportDraw, MergedConfiguration mergedConfiguration, Rect backDropFrame,
                boolean forceLayout, boolean alwaysConsumeNavBar, int displayId) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.dispatchResized(frame, overscanInsets, contentInsets, visibleInsets, stableInsets, outsets,
                        reportDraw, mergedConfiguration, backDropFrame, forceLayout, alwaysConsumeNavBar, displayId);
            }
        }
        ...
        private static int checkCallingPermission(String permission) {
            try {
                return ActivityManager.getService().checkPermission(permission, Binder.getCallingPid(),
            ...
            if (viewAncestor != null) {
                ...
                if (view != null) {
                    if (checkCallingPermission(Manifest.permission.DUMP) != PackageManager.PERMISSION_GRANTED) {
                        throw new SecurityException("Insufficient permissions to invoke" + " executeCommand() from pid="
     ...
     * this case, we need to force a relayout of the window to make sure we get the
     * correct bounds from window manager.
     */
    ...
    /**
     * Class for managing the accessibility interaction connection based on the
        ...
        public void ensureConnection() {
            ...
            if (!registered) {
                mAttachInfo.mAccessibilityWindowId = mAccessibilityManager.addAccessibilityInteractionConnection(
                        mWindow, mContext.getPackageName(), new AccessibilityInteractionConnection(ViewRootImpl.this));
            }
        }
     ...
     * AccessibilityManagerService to the latter can interact with the view
     * hierarchy in this ViewAncestor.
     */
    static final class AccessibilityInteractionConnection extends IAccessibilityInteractionConnection.Stub {
        ...
        @Override
        public void findAccessibilityNodeInfoByAccessibilityId(long accessibilityNodeId, Region interactiveRegion,
                int interactionId, IAccessibilityInteractionConnectionCallback callback, int flags,
                int interrogatingPid, long interrogatingTid, MagnificationSpec spec, Bundle args) {
            ViewRootImpl viewRootImpl = mViewRootImpl.get();
            if (viewRootImpl != null && viewRootImpl.mView != null) {
                viewRootImpl.getAccessibilityInteractionController()
                        .findAccessibilityNodeInfoByAccessibilityIdClientThread(accessibilityNodeId, interactiveRegion,
                                interactionId, callback, flags, interrogatingPid, interrogatingTid, spec, args);
            } else {
        ...
        @Override
        public void performAccessibilityAction(long accessibilityNodeId, int action, Bundle arguments,
                int interactionId, IAccessibilityInteractionConnectionCallback callback, int flags,
                int interrogatingPid, long interrogatingTid) {
            ViewRootImpl viewRootImpl = mViewRootImpl.get();
            if (viewRootImpl != null && viewRootImpl.mView != null) {
                viewRootImpl.getAccessibilityInteractionController().performAccessibilityActionClientThread(
                        accessibilityNodeId, action, arguments, interactionId, callback, flags, interrogatingPid,
                        interrogatingTid);
            } else {
                // We cannot make the call and notify the caller so it does not wait.
                try {
        ...
        public void findAccessibilityNodeInfosByViewId(long accessibilityNodeId, String viewId,
                Region interactiveRegion, int interactionId, IAccessibilityInteractionConnectionCallback callback,
                int flags, int interrogatingPid, long interrogatingTid, MagnificationSpec spec) {
            ViewRootImpl viewRootImpl = mViewRootImpl.get();
            if (viewRootImpl != null && viewRootImpl.mView != null) {
                viewRootImpl.getAccessibilityInteractionController().findAccessibilityNodeInfosByViewIdClientThread(
                        accessibilityNodeId, viewId, interactiveRegion, interactionId, callback, flags,
        ...
        @Override
        public void findAccessibilityNodeInfosByText(long accessibilityNodeId, String text, Region interactiveRegion,
                int interactionId, IAccessibilityInteractionConnectionCallback callback, int flags,
                int interrogatingPid, long interrogatingTid, MagnificationSpec spec) {
            ViewRootImpl viewRootImpl = mViewRootImpl.get();
            if (viewRootImpl != null && viewRootImpl.mView != null) {
                viewRootImpl.getAccessibilityInteractionController().findAccessibilityNodeInfosByTextClientThread(
                        accessibilityNodeId, text, interactiveRegion, interactionId, callback, flags, interrogatingPid,
        ...
        @Override
        public void findFocus(long accessibilityNodeId, int focusType, Region interactiveRegion, int interactionId,
                IAccessibilityInteractionConnectionCallback callback, int flags, int interrogatingPid,
                long interrogatingTid, MagnificationSpec spec) {
            ViewRootImpl viewRootImpl = mViewRootImpl.get();
            if (viewRootImpl != null && viewRootImpl.mView != null) {
                viewRootImpl.getAccessibilityInteractionController().findFocusClientThread(accessibilityNodeId,
                        focusType, interactiveRegion, interactionId, callback, flags, interrogatingPid,
                        interrogatingTid, spec);
            } else {
                // We cannot make the call and notify the caller so it does not wait.
                try {
        ...
        @Override
        public void focusSearch(long accessibilityNodeId, int direction, Region interactiveRegion, int interactionId,
                IAccessibilityInteractionConnectionCallback callback, int flags, int interrogatingPid,
                long interrogatingTid, MagnificationSpec spec) {
            ViewRootImpl viewRootImpl = mViewRootImpl.get();
            if (viewRootImpl != null && viewRootImpl.mView != null) {
                viewRootImpl.getAccessibilityInteractionController().focusSearchClientThread(accessibilityNodeId,
                        direction, interactiveRegion, interactionId, callback, flags, interrogatingPid,
                        interrogatingTid, spec);
            } else {
                // We cannot make the call and notify the caller so it does not wait.
                try {
