 ...
 * <ul>
 * <li>The application display area specifies the part of the display that may
 * contain an application window, excluding the system decorations. The
 * application display area may be smaller than the real display area because
 * the system subtracts the space needed for decor elements such as the status
 * bar. Use the following methods to query the application display area:
 * {@link #getSize}, {@link #getRectSize} and {@link #getMetrics}.</li>
 * <li>The real display area specifies the part of the display that contains
 * content including the system decorations. Even so, the real display area may
 * be smaller than the physical size of the display if the window manager is
 * emulating a smaller display using (adb shell wm size). Use the following
 * methods to query the real display area: {@link #getRealSize},
 * {@link #getRealMetrics}.</li>
 * </ul>
 * </p>
 * <p>
 * A logical display does not necessarily represent a particular physical
 * display device such as the built-in screen or an external monitor. The
 * contents of a logical display may be presented on one or more physical
 * displays according to the devices that are currently attached and whether
...
public final class Display {
     ...
     * pionux
     *
     * Fixed display id for the default external display. Currently only HDMI. The
     * default Display id for the desktop, which is the id of PIONUX Desktop's
     * "virtual" display.
     * 
     * @hide
     */
     ...
     * </p>
     * <p>
     * Secure (DRM) video decoders may allocate protected graphics buffers to
     * request that a hardware-protected path be provided between the video decoder
     * and the external display sink. If a hardware-protected path is not available,
     * then content stored in protected graphics buffers may not be composited.
     * </p>
     * <p>
     * An application can use the absence of this flag as a hint that it should not
     * use protected buffers for this display because the content may not be
     * visible. For example, if the flag is not set then the application may choose
     * not to show content on this display, show an informative error message,
     * select an alternate content stream or adopt a different strategy for decoding
     * content that does not rely on protected buffers.
     * </p>
     *
     * @see #getFlags
     */
     ...
     * capable of showing secure surfaces. It may also be capable of showing
     * {@link #FLAG_SUPPORTS_PROTECTED_BUFFERS protected buffers}.
     * </p>
     * <p>
     ...
     * {@link WindowManager.LayoutParams#FLAG_SECURE} window flag. Likewise, an
     * application creates a {@link SurfaceView} with a secure surface by calling
     * {@link SurfaceView#setSecure} before attaching the secure view to its
     * containing window.
     * </p>
     * <p>
     * An application can use the absence of this flag as a hint that it should not
     * create secure surfaces or protected buffers on this display because the
     * content may not be visible. For example, if the flag is not set then the
     * application may choose not to show content on this display, show an
     * informative error message, select an alternate content stream or adopt a
     * different strategy for decoding content that does not rely on secure surfaces
    ...
    /**
     * Display flag: Indicates that the display is private. Only the application
     * that owns the display and apps that are already on the display can create
     * windows on it.
     *
     * @see #getFlags
     */
     ...
     * automatically project their content to presentation displays to provide
     ...
     * <p>
     * This flag identifies displays that are circular, elliptical or otherwise do
     * not permit the user to see all the way to the logical corners of the display.
     * </p>
     *
     * @see #getFlags
     */
    ...
    /**
     * Display flag: Indicates that the display can show its content when non-secure
     * keyguard is shown.
     * <p>
     * This flag identifies secondary displays that will continue showing content if
     * keyguard can be dismissed without entering credentials.
     * </p>
     * <p>
     * An example of usage is a virtual display which content is displayed on
     * external hardware display that is not visible to the system directly.
     * </p>
     *
    ...
    /**
     * Display state: The display is dozing in a low power state; it is still on but
     * is optimized for showing system-provided content while the device is
     * non-interactive.
     *
     * @see #getState
    ...
    /**
     * Display state: The display is dozing in a suspended low power state; it is
     * still on but is optimized for showing static system-provided content while
     * the device is non-interactive. This mode may be used to conserve even more
     * power by allowing the hardware to stop applying frame buffer updates from the
     * graphics subsystem or to take over the display and manage it autonomously to
     * implement low power always-on display functionality.
     *
     * @see #getState
     * @see android.os.PowerManager#isInteractive
     */
    ...
    /**
     * Indicates that when display is removed, all its activities will be moved to
     * the primary display and the topmost activity should become focused.
     *
     * @hide
     */
    public static final int REMOVE_MODE_MOVE_CONTENT_TO_PRIMARY = 0;
    /**
     * Indicates that when display is removed, all its stacks and tasks will be
    ...
    /**
     * Internal method to create a display. The display created with this method
     * will have a static {@link DisplayAdjustments} applied. Applications should
     * use {@link android.view.WindowManager#getDefaultDisplay()} or
     * {@link android.hardware.display.DisplayManager#getDisplay} to get a display
     * object.
     *
     * @hide
     */
    ...
    /**
     * Internal method to create a display. The display created with this method
     * will be adjusted based on the adjustments in the supplied {@link Resources}.
     *
     * @hide
     */
    public Display(DisplayManagerGlobal global, int displayId, /* @NotNull */ DisplayInfo displayInfo, Resources res) {
        this(global, displayId, displayInfo, null /* daj */, res);
    }

    private Display(DisplayManagerGlobal global, int displayId, /* @NotNull */ DisplayInfo displayInfo,
            DisplayAdjustments daj, Resources res) {
        mGlobal = global;
        mDisplayId = displayId;
        mDisplayInfo = displayInfo;
        mResources = res;
        mDisplayAdjustments = mResources != null ? new DisplayAdjustments(mResources.getConfiguration())
     ...
     * (and rather fruitless) to continue using a {@link Display} object after the
     * display's demise.
     *
     ...
     *
     * @param outDisplayInfo The object to receive the copy of the display
     *                       information.
     * @return True if the display is still valid.
     * @hide
     */
     ...
     *
     * Each display has its own independent layer stack upon which surfaces are
     * placed to be managed by surface flinger.
     *
    ...
    /**
     * Gets the package name of the application that owns this display, or null if
     ...
     * </p>
     * <p>
     * 3. It can be different depending on the WindowManager to which the display
     * belongs.
     * </p>
     * <p>
     * - If requested from non-Activity context (e.g. Application context via
     * {@code (WindowManager) getApplicationContext().getSystemService(Context.WINDOW_SERVICE)})
     * it will report the size of the entire display based on current rotation and
     * with subtracted system decoration areas.
     * </p>
     * <p>
     * - If requested from activity (either using {@code getWindowManager()} or
     * {@code (WindowManager) getSystemService(Context.WINDOW_SERVICE)}) resulting
     * size will correspond to current app window size. In this case it can be
     * smaller than physical size in multi-window mode.
     * </p>
     * <p>
     * Typically for the purposes of layout apps should make a request from activity
     * context to obtain size available for the app content.
     * </p>
     *
     ...
     * size. This is basically the sizes you will see as the orientation changes,
     * taking into account whatever screen decoration there is in each rotation. For
     * example, the status bar is always at the top of the screen, so it will reduce
     * the height both in landscape and portrait, and the smallest height returned
     * here will be the smaller of the two.
     *
     ...
     * status bar, navigation bar, system bar, etc. It does <em>not</em> take into
     * account more transient elements like an IME soft keyboard.
     *
     * @param outSmallestSize Filled in with the smallest width and height that the
     *                        application will encounter, in pixels (not dp units).
     *                        The x (width) dimension here directly corresponds to
     *                        {@link android.content.res.Configuration#smallestScreenWidthDp
     *                        Configuration.smallestScreenWidthDp}, except the value
     *                        here is in raw screen pixels rather than dp units.
     *                        Your application may of course still get smaller space
     *                        yet if, for example, a soft keyboard is being
     *                        displayed.
     * @param outLargestSize  Filled in with the largest width and height that the
     *                        application will encounter, in pixels (not dp units).
     *                        Your application may of course still get larger space
     *                        than this if, for example, screen decorations like the
     *                        status bar are being hidden.
     */
    ...
    /**
     * @hide Return a rectangle defining the insets of the overscan region of the
     *       display. Each field of the rectangle is the number of pixels the
     *       overscan area extends into the display on that side.
     */
    public void getOverscanInsets(Rect outRect) {
        synchronized (this) {
            updateDisplayInfoLocked();
            outRect.set(mDisplayInfo.overscanLeft, mDisplayInfo.overscanTop, mDisplayInfo.overscanRight,
                    mDisplayInfo.overscanBottom);
        }
    }
     ...
     * {@link Surface#ROTATION_270 Surface.ROTATION_270}. For example, if a device
     * has a naturally tall screen, and the user has turned it on its side to go
     * into a landscape orientation, the value returned here may be either
     * {@link Surface#ROTATION_90 Surface.ROTATION_90} or
     * {@link Surface#ROTATION_270 Surface.ROTATION_270} depending on the direction
     * it was turned. The angle is the rotation of the drawn graphics on the screen,
     * which is the opposite direction of the physical rotation of the device. For
     * example, if the device is rotated 90 degrees counter-clockwise, to compensate
     * rendering will be rotated by 90 degrees clockwise and thus the returned value
     * here will be {@link Surface#ROTATION_90 Surface.ROTATION_90}.
     */
    @Surface.Rotation
     ...
     *
     * @deprecated This method is no longer supported. The result is always
     ...
     * <p>
     * This method only returns refresh rates for the display's default modes. For
     * more options, use {@link #getSupportedModes()}.
     *
    ...
    /**
     * @hide Get current remove mode of the display - what actions should be
     *       performed with the display's content when it is removed. Default
     *       behavior for public displays in this case is to move all activities to
     *       the primary display and make it focused. For private display - destroy
     *       all activities.
     *
    ...
    /**
     * Gets the app VSYNC offset, in nanoseconds. This is a positive value
     * indicating the phase offset of the VSYNC events provided by Choreographer
     * relative to the display refresh. For example, if Choreographer reports that
     * the refresh occurred at time N, it actually occurred at (N - appVsyncOffset).
     * <p>
     * Apps generally do not need to be aware of this. It's only useful for
    ...
    /**
     * This is how far in advance a buffer must be queued for presentation at a
     * given time. If you want a buffer to appear on the screen at time N, you must
     * submit the buffer before (N - presentationDeadline).
     * <p>
     * The desired presentation time for GLES rendering may be set with
     * {@link android.opengl.EGLExt#eglPresentationTimeANDROID}. For video decoding,
     * use {@link android.media.MediaCodec#releaseOutputBuffer(int, long)}. Times
     * are expressed in nanoseconds, using the system monotonic clock
    ...
    /**
     * Gets display metrics that describe the size and density of this display. The
     * size returned by this method does not necessarily represent the actual raw
     * size (native resolution) of the display.
     * <p>
     * 1. The returned size may be adjusted to exclude certain system decor elements
     * that are always visible.
     * </p>
     ...
     * - If requested from non-Activity context (e.g. Application context via
     * {@code (WindowManager) getApplicationContext().getSystemService(Context.WINDOW_SERVICE)})
     * metrics will report the size of the entire display based on current rotation
     * and with subtracted system decoration areas.
     * </p>
     * <p>
     * - If requested from activity (either using {@code getWindowManager()} or
     * {@code (WindowManager) getSystemService(Context.WINDOW_SERVICE)}) resulting
     * metrics will correspond to current app window metrics. In this case the size
     * can be smaller than physical size in multi-window mode.
     * </p>
     *
     ...
     *
     * @return The state of the display: one of {@link #STATE_OFF},
     *         {@link #STATE_ON}, {@link #STATE_DOZE}, {@link #STATE_DOZE_SUSPEND},
     *         or {@link #STATE_UNKNOWN}.
     */
    public int getState() {
        synchronized (this) {
            updateDisplayInfoLocked();
    ...
    public static boolean hasAccess(int uid, int flags, int ownerUid) {
        return (flags & Display.FLAG_PRIVATE) == 0 || uid == ownerUid || uid == Process.SYSTEM_UID || uid == 0;
    }
    ...
    public boolean isPublicPresentation() {
        return (mFlags & (Display.FLAG_PRIVATE | Display.FLAG_PRESENTATION)) == Display.FLAG_PRESENTATION;
    }
    ...
    public static final class Mode implements Parcelable {
        ...
        /**
         * Returns the physical width of the display in pixels when configured in this
         * mode's resolution.
         * <p>
         * Note that due to application UI scaling, the number of pixels made available
         * to applications when the mode is active (as reported by
         * {@link Display#getWidth()} may differ from the mode's actual resolution (as
         * reported by this function).
         * <p>
         * For example, applications running on a 4K display may have their UI laid out
         * and rendered in 1080p and then scaled up. Applications can take advantage of
         * the extra resolution by rendering content through a
         * {@link android.view.SurfaceView} using full size buffers.
         */
        ...
        /**
         * Returns the physical height of the display in pixels when configured in this
         * mode's resolution.
         * <p>
         * Note that due to application UI scaling, the number of pixels made available
         * to applications when the mode is active (as reported by
         * {@link Display#getHeight()} may differ from the mode's actual resolution (as
         * reported by this function).
         * <p>
         * For example, applications running on a 4K display may have their UI laid out
         * and rendered in 1080p and then scaled up. Applications can take advantage of
        ...
        @Override
        public String toString() {
            return new StringBuilder("{").append("id=").append(mModeId).append(", width=").append(mWidth)
                    .append(", height=").append(mHeight).append(", fps=").append(mRefreshRate).append("}").toString();
        }
     ...
     * types it supports and details about the desired luminance data.
     * <p>
     * You can get an instance for a given {@link Display} object with
    ...
    public static final class HdrCapabilities implements Parcelable {
        ...
        /** @hide */
        @IntDef({ HDR_TYPE_DOLBY_VISION, HDR_TYPE_HDR10, HDR_TYPE_HLG, })
         ...
         * @hide
         */
        public HdrCapabilities(int[] supportedHdrTypes, float maxLuminance, float maxAverageLuminance,
                float minLuminance) {
        ...
        /**
         * Gets the supported HDR types of this display. Returns empty array if HDR is
        ...
        /**
         * Returns the desired content max frame-average luminance data in cd/m2 for
         * this display.
         */
        ...
        public boolean equals(Object other) {
            ...
            return Arrays.equals(mSupportedHdrTypes, that.mSupportedHdrTypes) && mMaxLuminance == that.mMaxLuminance
                    && mMaxAverageLuminance == that.mMaxAverageLuminance && mMinLuminance == that.mMinLuminance;
        }
